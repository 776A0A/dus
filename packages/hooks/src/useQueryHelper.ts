import { isNullable } from '@dus/tools'
import { useHttp } from './useHttp'

type BaseState = ReturnType<typeof useHttp>

interface QueryOptions<T extends (...params: any[]) => Promise<any>> {
...
export const useQueryHelper = <ExtraState extends NormalObj>(
  ...
  const queryTemplate = <T extends (...params: any[]) => Promise<any>>(
    ...
    if (isNullable(options.preventParallel)) {
