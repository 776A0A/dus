...
describe('RoutesBuilder', () => {
  ...
  it('如果 temporary allow 为 true，那么就算没有权限也不会被删除', () => {
    const { getLength } = factory(
      [{ ...a, meta: { ...meta, [TA]: true } }],
      [b.path]
    )
  ...
  it('customFilter 返回 true，不删除相关路由', () => {
    const { getLength } = factory([{ ...a, meta }], [b.path], () => true)

    expect(getLength()).toBe(1)
  })

  it('customFilter 返回 false，删除相关路由', () => {
    const { getLength } = factory([{ ...a, meta }], [a.path], () => false)

    expect(getLength()).toBe(0)
  })

  it('customFilter 返回 undefined，交给默认逻辑处理', () => {
    const { getLength } = factory(
      [{ ...a, meta }, b],
      [a.path],
      () => undefined
